...
public class OldApkVersionDialog extends AppCompatDialog {
    Resources resources;
    ...
    TextView tv_message;
    Typeface typeface;
    OldApkVersionDialog thisDialog;
    Context context;

    public OldApkVersionDialog(Context context) {
        super(context);
        this.context = context;
        this.getWindow().requestFeature(1);
        this.setContentView(R.layout.pdlg_old_version_layot);
    ...
    private void setupViews_Base() {
        ...
        this.close_rotation_animation.setAnimationListener(new AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                OldApkVersionDialog.this.thisDialog.dismiss();
            }
        ...
        this.iv_icon.setOnTouchListener((v, event) -> {
            switch(event.getAction()) {
                ...
                case 1:
                    v.setAlpha(1.0F);
                    if (OldApkVersionDialog.this.icon_animation) {
                        v.startAnimation(OldApkVersionDialog.this.close_rotation_animation);
                    }
    ...
    public OldApkVersionDialog setGravity(int gravity) {
        this.getWindow().setGravity(gravity);
        return this;
    }

    public OldApkVersionDialog addButton(String text, Integer textColor, Integer backgroundColor, PrettyDialogCallback callback) {
        OldApkVerionDialogButton button = new OldApkVerionDialogButton(this.context, text, textColor, backgroundColor, this.typeface, callback);
    ...
    public OldApkVersionDialog setTitle(String text) {
    ...
    public OldApkVersionDialog setTitleColor(Integer color) {
        this.tv_title.setTextColor(this.context.getResources().getColor(color == null ? R.color.black : color));
        return this;
    }

    public OldApkVersionDialog setMessage(String text) {
        if (text.trim().length() > 0) {
    ...
    public OldApkVersionDialog setMessageColor(Integer color) {
        this.tv_message.setTextColor(this.context.getResources().getColor(color == null ? R.color.black : color));
        return this;
    }

    public OldApkVersionDialog setIcon(Integer icon) {
    ...
    public OldApkVersionDialog setIconTint(Integer color) {
        if (color == null) {
    ...
    public OldApkVersionDialog setIconCallback(final PrettyDialogCallback callback) {
        this.iv_icon.setOnTouchListener((OnTouchListener)null);
        if (callback != null) {
    ...
    public OldApkVersionDialog setIcon(Integer icon, Integer iconTint, final PrettyDialogCallback callback) {
        this.icon_animation = false;
        this.iv_icon.setImageResource(icon == null ? R.drawable.pdlg_icon_close : icon);
        if (iconTint == null) {
            this.iv_icon.setColorFilter((ColorFilter)null);
    ...
    public OldApkVersionDialog setTypeface(Typeface tf) {
        this.typeface = tf;
        this.tv_title.setTypeface(tf);
        this.tv_message.setTypeface(tf);

        for(int i = 0; i < this.ll_buttons.getChildCount(); ++i) {
            OldApkVerionDialogButton button = (OldApkVerionDialogButton) this.ll_buttons.getChildAt(i);
    ...
    public OldApkVersionDialog setAnimationEnabled(boolean enabled) {
        if (enabled) {
            this.getWindow().getAttributes().windowAnimations = R.style.pdlg_default_animation;
        } else {
